# Nexus for Slack — DevPost Submission Description
...
**Nexus for Slack** is a fully autonomous AI agent integrated natively into Slack via Socket Mode. It brings the complete power of the [Nexus Engine](https://github.com/EngineerProjects/nexus-engine) runtime directly into your workspace.

**Core capabilities:**
- **Slack workspace search** — Uses Slack's Real-Time Search API (`assistant.search.context`) to find messages, files, and channels across your organization. Ask Nexus to find a document, recall a past decision, or summarize what was discussed in a channel.
...
Nexus for Slack is built on top of the open-source **Nexus Engine** (`nexus-engine`), a Go runtime for autonomous AI agents with 60+ built-in tools, 15 LLM provider adapters, a permission engine, session persistence, and an MCP client.

**Architecture:**
- **Transport**: Slack Socket Mode (WebSocket) — no public endpoint, no webhooks, works behind a firewall.
- **Session model**: One persistent Nexus session per Slack channel. Context accumulates across messages. Sessions survive bot restarts via SQLite.
- **Real-Time Search API**: A custom `slack_search` tool registered with the Nexus agent calls `assistant.search.context` to search the workspace in real time. Results include message context, file metadata, and channel info.
- **MCP integration**: The bot reads `~/.config/nexus-slack/mcp.json` at startup and connects to any configured MCP server (stdio, HTTP, SSE, WebSocket transports). Falls back to the CLI's `mcp.json` for shared servers.
...
- **Runtime isolation**: The bot runs under `~/.config/nexus-slack/` — completely separate from other Nexus clients. Memory, sessions, and artifacts are co-located per session.

**Stack**: Go 1.24, slack-go v0.26.0, SQLite, Nexus Engine SDK, GLM-4.5 (z-ai).
...
- **`ask_user_question` without stdin**: The Nexus engine normally prompts via stdin. For the autonomous Slack context, we implemented a `PromptFn` hook that routes choice/confirm prompts to Block Kit buttons and text prompts to thread replies — with 5-minute timeouts.
...
- Real-Time Search API fully integrated as a first-class Nexus tool
...
- **Nexus AI**: Managed cloud hosting with team workspaces, governance, and a skill marketplace

## Built with

`Go` · `Slack Socket Mode` · `Slack Real-Time Search API (assistant.search.context)` · `MCP (Model Context Protocol)` · `SQLite` · `GLM-4.5 (z-ai)` · `Nexus Engine (open source)`

// Package agent is the core orchestration layer for Nexus AI agents.
//
...
var userAgent = fmt.Sprintf("Nexus/%s", version.Version)
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `nexus run` against a
	...
	// event, so non-interactive clients (e.g. `nexus run`) don't
...
// RunID, e.g. `nexus run`, would otherwise hang). Uncanceled calls without
...
// that RunID: `nexus run` ignores live message events and exits only on a
...
// so callers waiting on those RunIDs (e.g. `nexus run`) are not left
...
// caller waiting on RunComplete (e.g. `nexus run` with a RunID) always
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil {
		...
		if a.canceledBySeq(call.SessionID, call.Accepted.seq) {
			...
			// RunComplete for this RunID (e.g. `nexus run`, which
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "nexus auth" to re-authenticate.`)
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("NEXUS_DISABLE_ANTHROPIC_CACHE")); t {

...
var nexusGlobalIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	...
	if err != nil {
		if !os.IsNotExist(err) {
			slog.Debug("Failed to read nexus global ignore file", "path", name, "error", err)
		}
		return nil
	}
...
func (dl *directoryLister) getCombinedMatcher(dir string) gitignore.Matcher {
	return dl.combinedMatchers.GetOrSet(dir, func() gitignore.Matcher {
		...
		// Add global ignore patterns (git core.excludesFile + nexus global ignore).
		allPatterns = append(allPatterns, gitGlobalIgnorePatterns()...)
		allPatterns = append(allPatterns, nexusGlobalIgnorePatterns()...)

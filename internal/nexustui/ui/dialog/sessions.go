...
func NewSessions(com *common.Common, selectedSessionID string) (*Session, error) {
	...
	s.keyMap.Back = key.NewBinding(key.WithKeys("left"), key.WithHelp("←", "close"))
	...
	case tea.KeyPressMsg:
		...
		case sessionsModeDeleting:
			...
			case key.Matches(msg, s.keyMap.ConfirmDelete):
				...
				return action
			case key.Matches(msg, s.keyMap.Back), key.Matches(msg, s.keyMap.CancelDelete):
				s.sessionsMode = sessionsModeNormal
				s.list.SetItems(sessionItems(s.com.Styles, sessionsModeNormal, s.sessions...)...)
			}
		case sessionsModeUpdating:
			...
			case key.Matches(msg, s.keyMap.ConfirmRename):
				...
				s.list.SetItems(sessionItems(s.com.Styles, sessionsModeNormal, s.sessions...)...)
				return action
			case key.Matches(msg, s.keyMap.Back), key.Matches(msg, s.keyMap.CancelRename):
				s.sessionsMode = sessionsModeNormal
				s.list.SetItems(sessionItems(s.com.Styles, sessionsModeNormal, s.sessions...)...)
			default:
		...
		default:
			switch {
			case key.Matches(msg, s.keyMap.Back), key.Matches(msg, s.keyMap.Close):
				return ActionClose{}
...
func (s *Session) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	// rc.Title intentionally empty — title prepended below after mode switch to avoid ////.
	switch s.sessionsMode {
	case sessionsModeDeleting:
	...
	// Prepend plain title (no //// gradient). Uses the mode-specific TitleStyle
	// already set above so delete/rename modes render in their own border color.
	rc.Parts = append([]string{rc.TitleStyle.Render("Sessions")}, rc.Parts...)

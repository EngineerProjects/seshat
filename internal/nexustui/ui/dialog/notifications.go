...
func NewNotifications(com *common.Common) *Notifications {
	...
	n.keyMap.Back = key.NewBinding(key.WithKeys("left"), key.WithHelp("←", "back"))
...
func (n *Notifications) HandleMsg(msg tea.Msg) Action {
	...
	case tea.KeyPressMsg:
		switch {
		case key.Matches(msg, n.keyMap.Back):
			return ActionOpenDialog{DialogID: CommandsID}
		case key.Matches(msg, n.keyMap.Close):
			return ActionClose{}
		case key.Matches(msg, n.keyMap.Previous):
			n.list.Focus()
			n.list.SelectPrevCyclic()
		case key.Matches(msg, n.keyMap.Next):
			n.list.Focus()
			n.list.SelectNextCyclic()
...
func (n *Notifications) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	n.list.SetSize(innerWidth, 9999)
	measuredContentH := n.list.TotalHeight()
	maxTermH := max(0, area.Dy()-t.Dialog.View.GetVerticalBorderSize())
	height := max(overhead+1, min(notificationsDialogMaxHeight, min(maxTermH, overhead+measuredContentH)))
	n.list.SetSize(innerWidth, max(1, height-overhead))
	...
	rc.Parts = []string{rc.TitleStyle.Render(orange.Render("Settings  ›  Commands  ›  Notification Style"))}

	rc.AddPart(t.Dialog.InputPrompt.Render(n.input.View()))
	rc.AddPart(sep)
	rc.AddPart(t.Dialog.SecondaryText.Render("  choose a notification style"))
	...
	rc.AddPart(t.Dialog.List.Height(n.list.Height()).Render(n.list.Render()))
...
func (n *NotificationItem) Render(width int) string {
	...
	st := ListItemStyles{
		...
		ItemFocused: n.t.Dialog.SelectedItem.
			...
			Foreground(n.t.Dialog.NormalItem.GetForeground()),

...
// nexus-engine provider. Must match the URL prefixes in config.go TestConnection.
...
// catwalkProviderType maps a nexus-engine provider ID to the matching catwalk.Type
...
// buildNexusProviders constructs the canonical []catwalk.Provider list from
// nexus-engine's SDK provider catalog, extended with the models in extra_models.json.
...
func buildNexusProviders() []catwalk.Provider {

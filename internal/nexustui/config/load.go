...
// so an unrelated nexus.json placed above the project is never picked
...
// fallback keeps Nexus from silently adopting state files placed above
// the current project.
...
// ProjectSkillsDir returns the default project directories for which Nexus
// will look for skills.
func ProjectSkillsDir(workingDir string) []string {
	return []string{
		filepath.Join(workingDir, ".agents/skills"),
		filepath.Join(workingDir, ".nexus/skills"),

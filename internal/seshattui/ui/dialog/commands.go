...
func NewCommands(com *common.Common, sessionID string, hasSession, hasTodos, hasQueue bool, customCommands []commands.CustomCommand, mcpPrompts []commands.MCPPrompt) (*Commands, error) {
	...
	c.keyMap.Back = key.NewBinding(key.WithKeys("left"), key.WithHelp("←", "back"))
...
func (c *Commands) HandleMsg(msg tea.Msg) Action {
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(msg, c.keyMap.Previous):
			c.list.Focus()
			c.list.SelectPrevCyclic()
		case key.Matches(msg, c.keyMap.Next):
			c.list.Focus()
			c.list.SelectNextCyclic()
...
func (c *Commands) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	// Refresh system commands if window width changed (some items are width-aware).
	...
	// Fixed height budget — same formula as Settings root view.
	...
	const subBlock = 3 // sep(1) + subtitle(1) + blank(1)
	const sepAbove = 1
	...
	c.list.SetSize(innerWidth, finalContentH)

	sep := t.Header.Separator.Render(strings.Repeat("─", innerWidth))

	rc := NewRenderContext(t, width)
	// Bypass rc.Title — plain bold breadcrumb with optional radio selector inline.
	orange := lipgloss.NewStyle().Bold(true).Foreground(t.Logo.FieldColor)
	radioInfo := commandsRadioView(t, c.selected, len(c.customCommands) > 0, len(c.mcpPrompts) > 0)
	titleContent := orange.Render("Settings  ›  Commands")
	if radioInfo != "" {
	...
	rc.AddPart(sep)
	rc.AddPart(t.Dialog.SecondaryText.Render("  choose a command"))
...
func (c *Commands) defaultCommands() []*CommandItem {
	...
	// Copy last message is only useful when there are prior messages.
	if c.hasSession {
		commands = append(commands, NewCommandItem(c.com.Styles, "copy_last_message", "Copy Last Message", "ctrl+u", ActionCopyLastMessage{}))
	}

	// Verbose agent steps toggle shows/hides detailed step narration during runs.
	verboseLabel := "Enable Verbose Agent Steps"
	cfg2 := c.com.Config()
	if cfg2 != nil && cfg2.Options != nil && cfg2.Options.TUI.VerboseSteps {
		verboseLabel = "Disable Verbose Agent Steps"
	}
	commands = append(commands, NewCommandItem(c.com.Styles, "toggle_verbose_steps", verboseLabel, "", ActionToggleVerboseSteps{}))

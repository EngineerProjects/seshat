...
// seshatGlobalIgnorePatterns returns patterns from the user's
// runtime-root ignore file (for the TUI this defaults to ~/.config/seshat-tui/ignore).
var seshatGlobalIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	name := runtimepath.Join("", "ignore")
	bts, err := os.ReadFile(name)
	if err != nil {
		if !os.IsNotExist(err) {
			slog.Debug("Failed to read seshat global ignore file", "path", name, "error", err)
		}
		return nil
	}
...
type directoryLister struct {
	// dirPatterns caches parsed patterns from .gitignore/.nexusignore for each directory.
...
// .gitignore and .nexusignore files. Results are cached.
...
func (dl *directoryLister) getCombinedMatcher(dir string) gitignore.Matcher {
	return dl.combinedMatchers.GetOrSet(dir, func() gitignore.Matcher {
		...
		// Add global ignore patterns (git core.excludesFile + seshat global ignore).
		allPatterns = append(allPatterns, gitGlobalIgnorePatterns()...)
		allPatterns = append(allPatterns, seshatGlobalIgnorePatterns()...)

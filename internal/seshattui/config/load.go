...
func Load(workingDir, dataDir string, debug bool) (*ConfigStore, error) {
	...
	// Default to transparent (terminal's own background) for all users.
	// The user can switch to a solid background via Settings › Theme.
...
// so an unrelated seshat.json placed above the project is never picked
...
func GlobalConfig() string {
	if seshatGlobal := os.Getenv("SESHAT_GLOBAL_CONFIG"); seshatGlobal != "" {
		return filepath.Join(seshatGlobal, configFileName())
	}
...
func GlobalCacheDir() string {
	if seshatCache := os.Getenv("SESHAT_CACHE_DIR"); seshatCache != "" {
		return seshatCache
	}
...
func GlobalConfigData() string {
	if seshatData := os.Getenv("SESHAT_GLOBAL_DATA"); seshatData != "" {
		return filepath.Join(seshatData, configFileName())
	}
	return runtimepath.Join("", configFileName())
}
...
// fallback keeps Seshat from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if seshatSkills := os.Getenv("SESHAT_SKILLS_DIR"); seshatSkills != "" {
		return []string{seshatSkills}
	}
...
// ProjectSkillsDir returns the default project directories for which Seshat
// will look for skills.
func ProjectSkillsDir(workingDir string) []string {
	return []string{
		filepath.Join(workingDir, ".agents/skills"),
		filepath.Join(workingDir, ".seshat/skills"),

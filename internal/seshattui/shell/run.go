...
// Seshat handler stack. Shared by the stateless [Run] entrypoint and the
...
// regardless of the caller's environment because Seshat shells are never
...
//  1. builtins first (so Seshat's in-process jq wins over any PATH binary);
...
func standardHandlers(blockFuncs []BlockFunc) []func(next interp.ExecHandlerFunc) interp.ExecHandlerFunc {
	...
	// coreutils (moreinterp) disabled — not used outside Windows
	_ = useGoCoreUtils
	return handlers
}

// builtinHandler returns middleware that dispatches recognized Seshat

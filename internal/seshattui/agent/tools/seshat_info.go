...
//go:embed seshat_info.md
var seshatInfoDescription string
...
func NewSeshatInfoTool(
	...
	t, _ := tool.NewBuilder(SeshatInfoToolName).
		WithDescription(seshatInfoDescription).
		ReadOnly().
		ConcurrencySafe().
		NoPermission().
		WithHandler(func(_ context.Context, _ tool.CallInput, _ tool.ToolUseContext) (tool.CallResult, error) {
			return tool.NewTextResult(buildSeshatInfo(cfg, lspManager, allSkills, activeSkills, skillTracker)), nil
		}).
		Build()
	return t
}

func buildSeshatInfo(cfg *config.ConfigStore, lspManager *lsp.Manager, allSkills []*skills.Skill, activeSkills []*skills.Skill, skillTracker *skills.Tracker) string {

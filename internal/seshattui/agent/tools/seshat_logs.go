...
//go:embed seshat_logs.md.tpl
var seshatLogsDescriptionTmpl []byte

var seshatLogsDescriptionTpl = template.Must(
	template.New("seshatLogsDescription").
		Parse(string(seshatLogsDescriptionTmpl)),
)

type seshatLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func seshatLogsDescription() string {
	return renderTemplate(seshatLogsDescriptionTpl, seshatLogsDescriptionData{
...
func NewSeshatLogsTool(logFile string) tool.Tool {
	t, _ := tool.NewBuilder(SeshatLogsToolName).
		WithDescription(seshatLogsDescription()).
		ReadOnly().
		ConcurrencySafe().
		NoPermission().
		WithHandler(func(_ context.Context, input tool.CallInput, _ tool.ToolUseContext) (tool.CallResult, error) {
			...
			result := runSeshatLogs(logFile, SeshatLogsParams{Lines: int(lines)})
...
// runSeshatLogs reads and formats the last N log entries from the given file.
func runSeshatLogs(logFile string, params SeshatLogsParams) string {
